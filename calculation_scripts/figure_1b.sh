...
# Script for generating the data needed for Figure 1b. Some redundancy with the data needed for other figures.
...
noise_array=({0.02,0.08,0.2,0.4,1})
fraction_array=({1e-05,0.0001,0.001,0.01,0.1})
seed_array=({1,2,3,4,5})  # 5 x 5 x 5
fraction=${fraction_array[$(($SLURM_ARRAY_TASK_ID / 25))]}
noise=${noise_array[$(($SLURM_ARRAY_TASK_ID % 25 / 5))]}
